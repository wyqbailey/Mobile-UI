...
$.widget( "mobile.page", $.mobile.widget, {
	options: {
		backBtnText: "返回",//返回按钮文字
		addBackBtn: false,//添加返回按钮
		backBtnTheme: null,//返回按钮样式
		degradeInputs: {//各式输入框的降级处理，需要在手机上测试实际效果
			color: true,
			date: true,
			datetime: true,
			"datetime-local": true,
	...
	_create: function() {
		...
		var $dataEls = $elem.find( ":jqmData(role)" ).andSelf().each(function() {
			...
			if ( role === "header" || role === "footer" ) {
				$this.addClass( "ui-bar-" + (theme || $this.parent( ":jqmData(role='page')" ).jqmData( "theme" ) || "c") );
